...
func GET_BOOK_INFORMATION(NovelId string) Template.BookInfo {
	...
	api.Get("novels/"+NovelId).Add("expand", expand).NewRequests().Unmarshal(&BookInfoType)
	return BookInfoType
}

func GET_CATALOGUE(NovelID string) Template.Catalogue {
	var CatalogueType Template.Catalogue
	api.Get(fmt.Sprintf("novels/%v/dirs", NovelID)).Add("expand", "originNeedFireMoney").NewRequests().
...
func GET_CHAPTER_CONTENT(chapter_id string) Template.Content {
	...
	api.Get("Chaps/"+chapter_id).Add("expand", "content").NewRequests().Unmarshal(&ContentType)
	return ContentType
}

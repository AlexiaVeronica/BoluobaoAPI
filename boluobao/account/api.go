...
func GET_ACCOUNT_INFORMATION() Template.Account {
	...
	api.Get("user").NewRequests().AddAll(params).Unmarshal(&Account)
	return Account
}

//func GET_ACCOUNT_MONEY_INFORMATION() Template.Account {
//	var Account Template.Account
//	api.Get("user/money").NewRequests().Unmarshal(&Account)
//	return Account
//}

func GET_ACCOUNT_IP() Template.AccountIP {
	var AccountIP Template.AccountIP
	api.Get("position").NewRequests().Unmarshal(&AccountIP)
	return AccountIP
}
...
func GET_USER_INFORMATION(AccountId string) Template.Users {
	...
	api.Get("users/" + AccountId).NewRequests().AddAll(params).Unmarshal(&Users)
	return Users
}
...
func GET_USER_WORKS(AccountId string) Template.AuthorInfo {
	...
	api.Get("users/" + AccountId + "/novels").NewRequests().AddAll(params).Unmarshal(&AuthorInfo)
	return AuthorInfo
}
...
func GET_USER_COMMENT(AccountId string, page int) Template.AccountComment {
	...
	api.Get("users/" + AccountId + "/dynamics").NewRequests().AddAll(params).Unmarshal(&AccountComment)
...
func LOGIN_ACCOUNT(username string, password string) string {
	...
	response := api.Post("sessions").AddAll(params).NewRequests().Unmarshal(&Template.Login)
